...
# Predefined rule-based tips fallback
DEFAULT_TIPS = [
...
def get_rule_based_analysis(expenses, goals):
    # Perform math-driven local financial calculations
    ...
    # High-percentage categories detection
    ...
    try:
        ...
        if Config.OPENAI_API_KEY and Config.OPENAI_API_KEY.strip() != "":
            try:
                ...
                # Format a highly detailed analytical prompt
                prompt = (
        ...
        # Execute standard rule engine fallback
...
def predict_spending(current_user_id):
    try:
        ...
        # Run linear trend and moving average estimation locally
        ...
        for cat, amt in category_sums.items():
            # Estimate next month category cost based on average monthly weight
        ...
        # Core mathematical projection logic
